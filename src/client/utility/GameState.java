...
import client.exceptions.UnknownBuildingException;
import client.exceptions.UnknownCombatantException;
import client.model.army.*;
import client.model.buildings.*;
import client.model.habitants.GoldMiner;
...
import client.utility.timers.ProductionBuildingCollectionTimer;
...
public class GameState {
    public synchronized static String toJSONString(Village village) {
    ...
    public synchronized static void save(String str, String filePath) {
    ...
    public synchronized static void save(Village village, String filePath) {
        File file = new File(filePath);

        try(FileWriter fw = new FileWriter(file)) {
            fw.write(toJSONString(village));
        } catch (IOException e) {
            e.printStackTrace();
            System.err.println("Couldn't save!");
        }
    ...
    public synchronized static Village load(String str) {
        JSONObject obj = new JSONObject(str);
        return load(obj);
    }

    public synchronized static Village load(JSONObject obj) {
        Gold gold;
        Iron iron;
        Wood wood;
        Food food;
